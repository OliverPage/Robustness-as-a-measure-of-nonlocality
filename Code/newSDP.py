...
# -*- coding: utf-8 -*-
"""
Created on Fri Jul 29 13:22:13 2022
...
def parallel_data_production(no_games, batch_no=0):
    ...
    with open("ME_out/New SDP, max entangled epsilons, dims = {}, measures = {}, no games {}, batch {}.npy".format(no_dimensions, no_measurements, no_games, batch_no), "wb") as f:
...
if mode == 'Calculate':
    if __name__ == '__main__':
        ...
        for batch_no in range(no_batches):
            with open("ME_out/New SDP, max entangled epsilons, dims = {}, measures = {}, no games {}, batch {}.npy".format(no_dimensions, no_measurements, no_games, batch_no), "rb") as f:
                epsilon_max_entangled = np.load(f)
                epsilon_max_entangled_all = np.append(epsilon_max_entangled_all, epsilon_max_entangled)
                f.close()


        with open("New SDP, max entangled epsilons, dims = {}, measures = {}, no games {}, batch all.npy".format(no_dimensions, no_measurements, no_measures), "wb") as f:
            np.save(f, epsilon_max_entangled_all)
            f.close()


        no_games = no_measures
        write_stats("New SDP, Robustness, dims = {}, measures = {}, no games {}, batch all.txt".format(no_dimensions, no_measurements, no_measures), epsilon_max_entangled_all)
        plot(epsilon_max_entangled_all, no_measures, savename="New SDP, Robustness")
